...
class Consulta_model extends CI_Model
{
	...
	{
		$sql=$this->db->query('select * from maestra');
		if($sql->num_rows()>0){
	...
	public function guardar()
	{
		...
		$this->db->insert('maestra',$data);
	}
	public function editar_m()
	{
		$sql='select*from maestra where Id_estudiante=?';
	...
	public function actualizar_m()
	{
		...
		$this->db->update('maestra',$data);
	}
	public function borrar_m()
	{
		$id_estudiante=$_GET['ID_ESTUDIANTE'];
		$this->db->delete('maestra',array('id_estudiante'=>$id_estudiante));

	}
	public function informacion_m()
	{	
		//$sql=$this->db->query('select Nombres,Apellidos from maestra');
		$Id_estudiante=$_GET['ID_ESTUDIANTE'];
		$sql='select*from maestra where id_estudiante=?';
